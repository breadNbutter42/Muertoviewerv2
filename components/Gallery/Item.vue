<script setup>
import AOS from 'aos'
const props = defineProps(['data', 'loading', 'selected', 'grayscale'])

const backgroundColor = computed(() => {
  if (props.data.attributes[0].trait_type === 'Background') {
    switch (props.data.attributes[0].value) {
    case 'Light Blue':
      return 'bg-[#add8e6]'
    case 'Light Purple':
      return 'bg-[#e2d1f9]'
    case 'Orange':
      return 'bg-[#eea47f]'
    case 'Purple Sky':
      return 'bg-gradient-to-b from-[#ba92f1] via-[#decaf9] to-[#f0f2fe]'
    case 'Tosca':
      return 'bg-[#00a79f]'
    case 'Yellow':
      return 'bg-[#efc050]'
    }
  } else if (props.data.attributes[0].trait_type === 'One of One') {
    return 'bg-zinc-100/[.5] dark:bg-zinc-800'
  }
})
...
<template>
  <div :class="`relative w-full cursor-pointer hover:scale-[1.03] transition-all rounded-xl overflow-hidden shadow-xl shadow-black/[.15] dark:shadow-black/[.45]`">
    <div :class="`w-full ${backgroundColor} ${grayscale ? 'grayscale' : 'grayscale-0'}`">
      <div data-aos="fade-up" data-aos-once="false">
        <img :src="data.image" :class="`w-48 mx-auto`" :loading="loading ?? 'lazy'">
      </div>
    </div>
    <div class="absolute top-1 left-1 uppercase text-center  leading-none font-black font-anton text-white text-[38px] opacity-80 mix-blend-lighten itemstroke">
      <span class="mr-[2px]">#</span>{{ data.id }}
    </div>
    <div v-if="selected" class="absolute top-0 right-0 text-green-500 h-full w-full bg-white/[.10] pointer-events-none">
      <svg viewBox="0 0 20 20" fill="none" class="absolute top-2 right-2 w-6" xmlns="http://www.w3.org/2000/svg">
